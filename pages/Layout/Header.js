...
const  Header =()=> {
  ...
  return (
    <>
    <header className=" px-4  bg-primary flex flex-col absolute" >
      {(isVisible) ?
       ( <button onClick={toggleMenu} className="flex  w-full justify-end min-height-20 bg-primary  text-white p-4 ">
        ...
        ( <>
            <button onClick={toggleMenu} className=" fixed flex w-full justify-end min-height-20 bg-primary float-end text-white p-4 ">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
              </svg>
            </button>
            <ul className=' fixed transition-all w-full left-2/4 -translate-x-1/2 bg-secondry text-center text-sm items-center  h-full'>
